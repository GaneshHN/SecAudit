...
export default function Settings() {
  ...
  const [policies, setPolicies] = useState(() => {
    try {
      const saved = localStorage.getItem('secAuditPolicies')
      return saved ? JSON.parse(saved) : MOCK_POLICIES
    } catch {
  ...
  const handleSave = () => {
    setIsSaving(true)
    try {
      localStorage.setItem('secAuditPolicies', JSON.stringify(policies))
    } catch {
